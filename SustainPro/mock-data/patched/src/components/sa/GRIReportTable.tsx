...
export function GRIReportTable({ projectName, assignedBUs = [], reportingYear = 2025, projectId }: GRIReportTableProps) {
  ...
  useEffect(() => {
    const fetchAllBUData = async () => {
      ...
      // Fetch data for each business unit in parallel — direct Supabase query
      const promises = assignedBUs.map(async (buId) => {
        try {
          ...
          if (data && data.length > 0) {
            ...
            dataMap.set(buId, {
              businessUnitId: dbRow.business_unit_id,
              businessUnitName: buMeta?.name || '',
              calculatedData: dbRow.calculated_data || []
            });
          }
        } catch (err) {
